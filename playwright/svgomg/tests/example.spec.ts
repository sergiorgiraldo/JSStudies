...
test('verify menu items', async ({ page }) => {
    await expect(page.locator('.menu ul li')).toHaveText([
        'Open SVG',
        'Paste markup',
        'Demo',
        'Contribute'
    ...
    await expect(page.locator('.menu .app-title svg')).toHaveClass('logo');
});
...
test.describe('demo tests', () => {
    ...
    test('download result', async ({ page }) => {
        ...
        // const result = fs.readFileSync(await download.saveAs('/tmp'), 'utf-8');
