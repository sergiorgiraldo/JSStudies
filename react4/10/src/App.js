...
export default function App() {
	...
	function toggleDice(which) {
		setDices((oldDice) =>
			oldDice.map((d) => {
				return d.id === which ? { ...d, isHeld: !d.isHeld } : d;
			})
		);
	}
	...
	function rollDices() {
		if (tenzies) {
			...
			setDices((oldDice) =>
				oldDice.map((d) => {
					return d.isHeld ? d : { ...d, value: getRandomDiceValue() };
				})
			);
			...
			if (tenzies) {
				...
				fetch("http://localhost:3100/stats", {
					...
					body: JSON.stringify({
						duration: Math.ceil(
							Math.round(
								new Date(newStat.endDate).getTime() -
									new Date(newStat.startDate).getTime()
							) / 1000
						),
						count: newStat.count
					})
					...
					.then(function () {
						setDbUpdated((oldValue) => oldValue + 1);
					})
					.catch(function (res) {
						console.log(res);
					});
